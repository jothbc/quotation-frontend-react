...
import { Container, Logo, Button, Content } from './styles';
...
const SignUp: React.FC = () => {
  ...
  const schema = Yup.object().shape({
    ...
    password2: Yup.string().required('Obrigatório confirmar a senha'),
    company: Yup.string().required('Infome sua Empresa'),
    CNPJ: Yup.string().required('Informe o CNPJ da empresa'),
    tel: Yup.string().required('Informe um número para contato'),
    city: Yup.string().required('Informe sua cidade'),
    state: Yup.string().required('Informe seu Estado'),
  });

  const handleSubmit = useCallback(async (data: IDataProps) => {
    try {
